...
output_image_file_location = current_path+y_data_file_name+'-'+x_data_file_name+'_integral.'+image_file_extension
...
if x_data.ndim > 1:
    ...
    for i in range(0,total_data_number):
        ...
        temp_y_integral_data = numpy.zeros(len(temp_x_data))
        for j in range(1,len(temp_x_data)):
            temp_y_integral_data[j] = scipy.integrate.simpson(y=temp_y_data[0:j],x=temp_x_data[0:j])
        ax1.plot(temp_x_data, temp_y_integral_data, color=plot_color, alpha=0.1)
        max_y_array[i] = numpy.max(temp_y_integral_data)
